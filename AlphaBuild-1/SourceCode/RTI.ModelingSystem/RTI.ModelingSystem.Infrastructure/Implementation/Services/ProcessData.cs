			...
			try
			{
				...
				if (usingTwoSources)
				{
					foreach (var avgCond in this.weeklyAverageCondDataSource1)
					{
						...
						int wtAvg = Convert.ToInt32(Math.Round(average * sourcePercentOne, 0, MidpointRounding.AwayFromZero));
					...
					{
						int average = avgCond.Value.Item2;
                        int wtAvg = Convert.ToInt32(Math.Round(average * sourcePercentTwo, 0, MidpointRounding.AwayFromZero));
						Tuple<int, int> wkWtAvg = new Tuple<int, int>(avgCond.Value.Item1, avgCond.Value.Item2);
