	...
	public class PredictiveModelService : IPredictiveModelService
	{
		...
		public Dictionary<double, double> CurrentSSConditions(double resinAge, double cleaningEffectiveness, double startingSaltSplit)
		{
			try
			{
				...
				if (afterCleaningWeek != null && afterCleaningWeek.Count > 0)
				{
		...
		public PriceData Thoughputchart(string customerId, double currentSaltSplit, double startingSaltSplit, double resinLifeExpectancy, int simulationconfidence, int numberSimulationIterations, string simMethod, int standardDeviationThreshold, double resinAge, double replacementLevel, double rticleaningLevel, string selectedTrain, bool donotReplaceResin)
		{
			try
			{
				...
				if (conductivitSource1.Count > 0)
				{
					...
					conductivitySourceArray1[0] = (from cond in conductivitSource1 select Convert.ToString(cond.dataID)).ToList();
					conductivitySourceArray1[1] = (from cond in conductivitSource1 select Convert.ToString(cond.cond)).ToList();
					conductivitySourceArray1[2] = (from cond in conductivitSource1 select Convert.ToString(cond.temp)).ToList();
					conductivitySourceArray1[3] = (from cond in conductivitSource1 select Convert.ToString(cond.measurment_date)).ToList();
					conductivitySourceArray1[4] = (from cond in conductivitSource1 select Convert.ToString(cond.sourceID)).ToList();
					conductivitySourceArray2[0] = (from cond in conductivitSource2 select Convert.ToString(cond.dataID)).ToList();
					conductivitySourceArray2[1] = (from cond in conductivitSource2 select Convert.ToString(cond.cond)).ToList();
					conductivitySourceArray2[2] = (from cond in conductivitSource2 select Convert.ToString(cond.temp)).ToList();
					conductivitySourceArray2[3] = (from cond in conductivitSource2 select Convert.ToString(cond.measurment_date)).ToList();
					conductivitySourceArray2[4] = (from cond in conductivitSource2 select Convert.ToString(cond.sourceID)).ToList();
					double sourceOnePercent = 100;
